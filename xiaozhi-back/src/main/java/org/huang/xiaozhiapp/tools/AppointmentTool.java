package org.huang.xiaozhiapp.tools;
...
public class AppointmentTool {
    ...
    public String bookAppointment(Appointment appointment) {
        //查找数据库中是否包含对应的预约记录
        Appointment appointmentDB = xiaozhiService.getAppointment(appointment);
        if (appointmentDB == null) {
            appointment.setId(null);//防止大模型幻觉设置了id
            if (xiaozhiService.insertAppointment(appointment)==1) {
                return "预约成功，并返回预约详情";
            } else {
                return "预约失败";
            }
    ...
    public String cancelAppointment(Appointment appointment){
        ...
        if(appointmentDB != null){
            //删除预约记录
            if(xiaozhiService.deleteAppointment(appointment)==1){
                return "取消预约成功";
            }else{
                return "取消预约失败";
            }
